...
#                 Real_beta[, 2],nn1)
R1_plot = cbind(Result_setting1[,8],Result_setting1[,23], Result_setting1[,28], Result_setting1[,33],
                Result_setting1[,18],Result_setting1[,38],Result_setting1[,43], Result_setting1[,48],
                Real_beta[, 2],nn1)
...
R1_plot[,5:8] = sqrt(R1_plot[,5:8])/sqrt(R1_plot[,5])
temp = rep(0,10)
...
PS1 <- ggplot(QD1,aes(x=n,y=Std_Err,group = Method,colour = Method,shape=Method)) + 
  geom_line(size=1)+geom_point(size=3)+labs(title = "Ratio of Standard Error\n",y ="") +
  scale_color_manual(values = cc) + scale_shape_manual(values = c(16,17,15,18)) +
