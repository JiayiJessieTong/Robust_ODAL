...
Generate = function(N,beta,nn){
  ...
  X1 = rnorm(N-3*nn)         #generate N numbers with N(0,1)
  X2 = rbinom(N-3*nn,1,0.3)  #generate N numbers with B(1,0.3)
  X3 = runif(N-3*nn,X2-1,1)    #generate N numbers with U(-1,1)
  X4 = rnorm(N-3*nn,0,2)     #generate N numbers with N(0,2)
  X = cbind(1,X1,X2,X3,X4)
  meanY = expit(X%*%beta)
  Y = rbinom(N-3*nn,1,meanY)
  ...
  X4.hetero = rnorm(nn,0,1)     #generate N numbers with N(0,1)
  ...
  X1.hetero.2 = rnorm(nn,0.5,1) 
  X2.hetero.2 = rbinom(nn,1,0.4)  #generate N numbers with B(1,0.4)
  X3.hetero.2 = runif(nn,X2.hetero.2-1.5,1.5)    #generate N numbers with U(-2,2)
  X4.hetero.2 = rnorm(nn,0,0.5)     #generate N numbers with N(0,3)
  ...
  # the heterogeneous cite
  X1.hetero.3 = rnorm(nn,0.2,1) 
  X2.hetero.3 = rbinom(nn,1,0.2)  #generate N numbers with B(1,0.4)
  X3.hetero.3 = runif(nn,X2.hetero.3-0.5,0.5)    #generate N numbers with U(-2,2)
  X4.hetero.3 = rnorm(nn,0,3)     #generate N numbers with N(0,3)
  X.hetero.3 = cbind(1,X1.hetero.3,X2.hetero.3,X3.hetero.3,X4.hetero.3)
  meanY.hetero.3 = expit(X.hetero.3%*%beta)
  Y.hetero.3 = rbinom(nn,1,meanY.hetero.3)
  
  Hetero.3 = cbind(X1.hetero.3, X2.hetero.3, X3.hetero.3, X4.hetero.3, Y.hetero.3)
    
  return(rbind(allMinisHetero, Hetero, Hetero.2, Hetero.3))
}
...
colnames(Result_setting1) = c("iter", "local beta0", "local beta1", "local beta2", "local beta3","local beta4",
