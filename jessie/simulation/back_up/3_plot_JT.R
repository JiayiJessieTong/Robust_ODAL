...
cc = c(brewer.pal(9,"Set2")[c(3,2,1)],"pink","purple")
#cc = c("brown2",cc1[1:3])
##########  process the data#################
R1_plot = cbind(Result_setting1[,3],Result_setting1[,8],Result_setting1[,23], Result_setting1[,28], Result_setting1[,33],
                Result_setting1[,13],Result_setting1[,18],Result_setting1[,38],Result_setting1[,43], Result_setting1[,48],
                Real_beta[, 2],nn1)

R1_plot[,1:5] = R1_plot[,1:5] - cbind(R1_plot[,11],R1_plot[,11],R1_plot[,11],R1_plot[,11],R1_plot[,11])
R1_plot[,6:10] = sqrt(R1_plot[,6:10])
temp = rep(0,12)
names(temp) =c("LOCAL","POOLED","ODAL1","ODAL2 mean", "ODAL2 median",
               "LOCAL var","POOLED var","ODAL1 var","ODAL2 mean var", "ODAL2 median var",
               "rl","n")
temp = cbind(t(t(temp)),t(R1_plot))
R1_plot = t(temp)
R1_plot = R1_plot[2:11,]

DR1_PE <- as.data.frame(R1_plot[,c(1:5,12)])
DR1_PS <- as.data.frame(R1_plot[,c(6:10,12)])
...
PE1 <- ggplot(QU1,aes(x=n,y=Bias,group = Method,colour = Method,shape=Method))+geom_line(size =1) +
  geom_point(size=3) + labs(title = "Bias\n",tag = "Setting A",y ="") +
   scale_color_manual(values = cc) + scale_shape_manual(values = c(16,17,15,18,19)) + 
...
##########  process the data#################
R2_plot = cbind(Result_setting2[,3],Result_setting2[,8],Result_setting2[,23], Result_setting2[,28], Result_setting2[,33],
                Result_setting2[,13],Result_setting2[,18],Result_setting2[,38],Result_setting2[,43], Result_setting2[,48],
                Real_beta[, 2],KK2)

R2_plot[,1:5] = R2_plot[,1:5] - cbind(R2_plot[,11],R2_plot[,11],R2_plot[,11],R2_plot[,11],R2_plot[,11])
R2_plot[,6:10] = sqrt(R2_plot[,6:10])
temp = rep(0,12)
names(temp) =c("LOCAL","POOLED","ODAL1","ODAL2 mean", "ODAL2 median",
               "LOCAL var","POOLED var","ODAL1 var","ODAL2 mean var", "ODAL2 median var",
               "rl","K")
...
DR2_PE <- as.data.frame(R2_plot[,c(1:5,12)])
DR2_PS <- as.data.frame(R2_plot[,c(6:10,12)])
...
##########  process the data#################
R3_plot = cbind(Result_setting3[,3],Result_setting3[,8],Result_setting3[,23], Result_setting3[,28], Result_setting3[,33],
                Result_setting3[,13],Result_setting3[,18],Result_setting3[,38],Result_setting3[,43], Result_setting3[,48],
                Real_beta[, 2],KK3)

R3_plot[,1:5] = R3_plot[,1:5] - cbind(R3_plot[,11],R3_plot[,11],R3_plot[,11],R3_plot[,11],R3_plot[,11])
R3_plot[,6:10] = sqrt(R3_plot[,6:10])
temp = rep(0,12)
names(temp) =c("LOCAL","POOLED","ODAL1","ODAL2 mean", "ODAL2 median",
               "LOCAL var","POOLED var","ODAL1 var","ODAL2 mean var", "ODAL2 median var",
               "rl","K")
...
DR3_PE <- as.data.frame(R3_plot[,c(1:5,12)])
DR3_PS <- as.data.frame(R3_plot[,c(6:10,12)])
...
##########  process the data#################
R4_plot = cbind(Result_setting4[,3],Result_setting4[,8],Result_setting4[,23], Result_setting4[,28], Result_setting4[,33],
                Result_setting4[,13],Result_setting4[,18],Result_setting4[,38],Result_setting4[,43], Result_setting4[,48],
                Real_beta[, 2],nn4)

R4_plot[,1:5] = R4_plot[,1:5] - cbind(R4_plot[,11],R4_plot[,11],R4_plot[,11],R4_plot[,11],R4_plot[,11])
R4_plot[,6:10] = sqrt(R4_plot[,6:10])
temp = rep(0,12)
names(temp) =c("LOCAL","POOLED","ODAL1","ODAL2 mean", "ODAL2 median",
               "LOCAL var","POOLED var","ODAL1 var","ODAL2 mean var", "ODAL2 median var",
               "rl","n")
...
DR4_PE <- as.data.frame(R4_plot[,c(1:5,12)])
DR4_PS <- as.data.frame(R4_plot[,c(6:10,12)])
...
pdf("/Users/Jessie/Desktop/000_UPenn_Research/000_project/000_ODAL/R_code/jessie/simulation/3hetero.pdf", height = 18, width = 14)
