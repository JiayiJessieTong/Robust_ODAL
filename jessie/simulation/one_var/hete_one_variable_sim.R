...
simu = function(beta,N,p,nn,a,b){
  ...
  X = Data[,1]
  Y = Data[,2]
  # nn is the number of each site, 
  # Xlocal = Data[1:nlocal,1]
  Xlocal = Data[1:nn,1]
  Ylocal = Data[1:nn,2]
  X.homo = Data[1:(N-nn),1]
  Y.homo = Data[1:(N-nn),2]
  ...
  beta_meta_fix <-metagen(beta_meta_list, se_meta_list,comb.fixed = TRUE,comb.random = FALSE,prediction=TRUE,sm="OR")$TE.fixed
  beta_meta_random <-metagen(beta_meta_list, se_meta_list,comb.fixed = TRUE,comb.random = FALSE,prediction=TRUE,sm="OR")$TE.random
  ...
  #output the outcome
  total = c(iter,
            beta_tilde1_mean_homo[2], beta_tilde1_median_homo[2],
            beta_tilde1_mean[2], beta_tilde1_median[2],
            beta_tilde2_mean_mean[2], beta_tilde2_mean_median[2], 
            beta_tilde2_median_mean[2], beta_tilde2_median_median[2],
...
for (K1 in c(10,50,100)){
  ...
  set.seed(1111)
  print("setting 1")
  # 1.1 setting: 100 each site
  out1_100 = replicate(Nsim,simu(beta_true,NN1[1],p1,nn = nn1[1],a = 0.5,b = 1.6))  #run func with Nsim times
  # 1.2 setting: 500 each site
  out1_500 = replicate(Nsim,simu(beta_true,NN1[2],p1,nn = nn1[2],a = 0.5,b = 1.6))  #run func with Nsim times
  # 1.3 setting: 1000 each site
  out1_1000 = replicate(Nsim,simu(beta_true,NN1[3],p1,nn = nn1[3],a = 0.5,b = 1.6))  #run func with Nsim times
  ...
  out2_100 = replicate(Nsim,simu(beta_true,NN1[1],p1,nn = 2*nn1[1],a = 0.5,b = 1.6))  #run func with Nsim times
  # 2.2 setting: 500 each site
  out2_500 = replicate(Nsim,simu(beta_true,NN1[2],p1,nn = 2*nn1[2],a = 0.5,b = 1.6))  #run func with Nsim times
  # 2.3 setting: 1000 each site
  out2_1000 = replicate(Nsim,simu(beta_true,NN1[3],p1,nn = 2*nn1[3],a = 0.5,b = 1.6))  #run func with Nsim times
  
  
  # setting 3: one hetero
  set.seed(1111)
  print("setting 3")
  # 3.1 setting: 100 each site
  out3_100 = replicate(Nsim,simu(beta_true,NN1[1],p1,nn = nn1[1],a = 0.5,b = 2.4))  #run func with Nsim times
  # 3.2 setting: 500 each site
  out3_500 = replicate(Nsim,simu(beta_true,NN1[2],p1,nn = nn1[2],a = 0.5,b = 2.4))  #run func with Nsim times
  # 3.3 setting: 1000 each site
  out3_1000 = replicate(Nsim,simu(beta_true,NN1[3],p1,nn = nn1[3],a = 0.5,b = 2.4))  #run func with Nsim times
  
  
  # setting 4: two hetero
  set.seed(1111)
  print("setting 4")
  # 4.1 setting: 100 each site
  out4_100 = replicate(Nsim,simu(beta_true,NN1[1],p1,nn = 2*nn1[1],a = 0.5,b = 2.4))  #run func with Nsim times
  # 4.2 setting: 500 each site
  out4_500 = replicate(Nsim,simu(beta_true,NN1[2],p1,nn = 2*nn1[2],a = 0.5,b = 2.4))  #run func with Nsim times
  # 4.3 setting: 1000 each site
  out4_1000 = replicate(Nsim,simu(beta_true,NN1[3],p1,nn = 2*nn1[3],a = 0.5,b = 2.4))  #run func with Nsim times
  
  # boxplot
  pdf(paste("/Users/Jessie/Desktop/000_UPenn_Research/000_project/000_ODAL/R_code/jessie/simulation/one_var/very_different_whole_new_one_var_results_","#sites_",K1,'.pdf',sep = ''),
      height = 10, width = 16)

  boxplot_func(1,out1_100,nn1[1],1,K1)
  boxplot_func(1,out1_500,nn1[2],1,K1)
  boxplot_func(1,out1_1000,nn1[3],1,K1)
  
  boxplot_func(1,out2_100,nn1[1],2,K1)
  boxplot_func(1,out2_500,nn1[2],2,K1)
  boxplot_func(1,out2_1000,nn1[3],2,K1)
  
  boxplot_func(1,out3_100,nn1[1],1,K1)
  boxplot_func(1,out3_500,nn1[2],1,K1)
  boxplot_func(1,out3_1000,nn1[3],1,K1)
  
  boxplot_func(1,out4_100,nn1[1],2,K1)
  boxplot_func(1,out4_500,nn1[2],2,K1)
  boxplot_func(1,out4_1000,nn1[3],2,K1)
