...
output1 = output_ODAL_median_AMI_ccae = readRDS("/Users/jiayito/Dropbox/2_Rui_and_Jessie/Summer_2019/4_ODAL_median/real_data_results/from_Martijn/output_ODAL_median_AMI_ccae.rds")
output_4 = readRDS("/Users/jiayito/Dropbox/2_Rui_and_Jessie/Summer_2019/4_ODAL_median/real_data_results/from_Martijn_2/output_ODAL_median_AMI_ccae_noJmdc.rds")
output_3 = readRDS("/Users/jiayito/Dropbox/2_Rui_and_Jessie/Summer_2019/4_ODAL_median/real_data_results/from_Martijn_2/output_ODAL_median_AMI_ccae_noJmdcNorMdcr.rds")
...
### forest plot
forest.plot <- function(output_matrix, title, with_label, margin_left){
  ...
  segments(output_matrix["beta_meta_CI_l"], 2.3,output_matrix["beta_meta_CI_u"], 2.3, lwd=4, col=color_list[3])
  segments(output_matrix["beta_meta"], 2.3-0.1,output_matrix["beta_meta"], 2.3+0.1, lwd=4, col=color_list[3])
  segments(output_matrix["beta_meta_CI_l"], 2.3-0.05,output_matrix["beta_meta_CI_l"], 2.3+0.05, lwd=4, col=color_list[3])
  segments(output_matrix["beta_meta_CI_u"], 2.3-0.05,output_matrix["beta_meta_CI_u"], 2.3+0.05, lwd=4, col=color_list[3])
  
  # segments for beta odal meta median
  segments(output_matrix["ODAL1_meta_median_CI_l_2"], 3.3,output_matrix["ODAL1_meta_median_CI_u_2"], 3.3, lwd=4, col = color_list[2])
  segments(output_matrix["ODAL1_meta_median"], 3.3-0.1,output_matrix["ODAL1_meta_median"], 3.3+0.1, lwd=4, col = color_list[2])
  segments(output_matrix["ODAL1_meta_median_CI_l_2"], 3.3-0.05,output_matrix["ODAL1_meta_median_CI_l_2"], 3.3+0.05, lwd=4, col = color_list[2])
  segments(output_matrix["ODAL1_meta_median_CI_u_2"], 3.3-0.05,output_matrix["ODAL1_meta_median_CI_u_2"], 3.3+0.05, lwd=4, col = color_list[2])
  
  # segments for beta odal meta mean
  segments(output_matrix["ODAL1_meta_mean_CI_l_2"], 4.3,output_matrix["ODAL1_meta_mean_CI_u_2"], 4.3, lwd=4, col = color_list[1])
  segments(output_matrix["ODAL1_meta_mean"], 4.3-0.1,output_matrix["ODAL1_meta_mean"], 4.3+0.1, lwd=4, col = color_list[1])
  segments(output_matrix["ODAL1_meta_mean_CI_l_2"], 4.3-0.05,output_matrix["ODAL1_meta_mean_CI_l_2"], 4.3+0.05, lwd=4, col = color_list[1])
  segments(output_matrix["ODAL1_meta_mean_CI_u_2"], 4.3-0.05,output_matrix["ODAL1_meta_mean_CI_u_2"], 4.3+0.05, lwd=4, col = color_list[1])
  
  if (with_label == TRUE){
    u = par("usr")
    text(u[1]-0.01,1.3,'Pooled',xpd=T,adj=1,cex=1.7)
    text(u[1]-0.01,2.3,'Meta\nAnalysis',xpd=T,adj=1,cex=1.7)
    text(u[1]-0.01,3.3,'Robust\nODAL',xpd=T,adj=1,cex=1.7)
    text(u[1]-0.01,4.3,'ODAL',xpd=T,adj=1,cex=1.7)
  }
}

forest.plot_2 <- function(output_matrix, subtitle){
  ...
  forest.plot(output_matrix[,2], title = paste("Obesity",subtitle), 
              with_label = TRUE, margin_left = 5.5)
  forest.plot(output_matrix[,3], title = paste("Alcohol dependence",subtitle), 
              with_label = FALSE, margin_left = 2)
  forest.plot(output_matrix[,4], title = paste("Hypertensive disorder",subtitle), 
              with_label = FALSE, margin_left = 2)
  dev.off()
  ...
  par(mfrow = c(1,3))
  forest.plot(output_matrix[,5], title = paste("Major depressive disorder",subtitle), 
              with_label = TRUE, margin_left = 5.5)
  forest.plot(output_matrix[,6], title = paste("Type 2 diabetes mellitus",subtitle), 
              with_label = FALSE, margin_left = 2)
  forest.plot(output_matrix[,7], title = paste("Hyperlipidemia",subtitle), 
              with_label = FALSE, margin_left = 2)
  dev.off()
}

output1_matrix = clean_output(output1,n=7)

# output1
pdf("/Users/jiayito/Dropbox/000_UPenn_Research/000_project/000_ODAL/R_code/jessie/dataAnalysis/AMI_CCAE_update.pdf", width = 12, height = 6)
forest.plot_2(output1_matrix, subtitle = "")
dev.off()
