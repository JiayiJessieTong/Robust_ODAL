...
######################################################################################
##### get data and identify largest site
library(tidyverse)
load("/Volumes/maltenform/Documents/Multi Site/cohort20.RData")
sites<-cohort %>% group_by(care_site_id) %>% summarise(n=n()) %>% arrange(desc(n)) %>% mutate(site=row_number()-1)
cohort <- cohort %>% inner_join(sites)

Xall <- cohort %>% dplyr::select(max_age,female,Asian,Black,Hispanic,Other)
Xall<-as.matrix(Xall)
site <- cohort$site  
...
for (i in 0:(len_site-1)){
  ...
  Asthma_results[[i+1]] <- compare.methods(Xall,Yall,site)
}
